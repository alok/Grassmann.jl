...
/-- Run the large oracle fuzz files `PREFIX_{f64,f32,opts,num,range,math}.tsv`. -/
def fuzzAll (pre : String) : IO (Nat × Nat) := do
  ...
  return (p1 + p2 + p3 + p4, f1 + f2 + f3 + f4)
...
def run : IO (Nat × Nat) := do
  ...
  let mut passed := 0
  let mut failed := 0
  for suite in [FloatShow.golden, Num.golden, Range.golden, Show.golden, Math.golden] do
