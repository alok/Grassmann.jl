...
/-!
The float printing AbstractAnalysis displays with (`JuliaBase.F64.showString`,
`F32.showString`, `F32.printString`), fuzzed against 7000+ `Float64` and 2000
`Float32` values printed by Julia (`oracle/golden/abstractanalysis/floatprint.json`).
-/

open Lean JuliaBase Tests.Golden
...
def suite : TestM Unit := do
  ...
  for e in jArr (jGet j "f64") do
    ...
    n64 := n64 + 1
    if F64.showString x != s then
      if bad64 == 0 then first64 := s!"{s} printed as {F64.showString x}"
      bad64 := bad64 + 1
  ...
  for e in jArr (jGet j "f32") do
    ...
    let s := jStr (jGet e "str")
    if F32.showString x != r || F32.printString x != s then
      if bad32 == 0 then first32 := s!"{r} printed as {F32.showString x}"
      bad32 := bad32 + 1
