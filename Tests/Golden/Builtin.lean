...
| `leibniz/storage` | construct | the dense vector from `(kind, grade, bits, T, native)` through the Leibniz storage orders (schema §7.1, incl. degenerate Couples), and the display of `Zero`, `One`, basis blades (`DirectSum.TensorBundle.bladeLabel`) | kind, values, strings |
