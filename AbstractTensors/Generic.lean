...
@[inline] def approx (a b : Float) : Bool := JuliaBase.F64.isapprox a b
...
def isapprox (a b : X) (atol : Float := 0) (rtol : Float := if atol > 0 then 0 else JuliaBase.F64.rtoldefault)
    (nans : Bool := false) : Bool :=
  let x := norm a
  let y := norm b
  (x.isFinite && y.isFinite && norm (a - b) ≤ JuliaBase.F64.max atol (rtol * JuliaBase.F64.max x y)) ||
...
@[inline] def log2 (t : X) : X := smul JuliaBase.F64.log2e (TensorRing.log t)

/-- Julia `log10(t) = log10(ℯ)·log(t)` (AT:383). -/
@[inline] def log10 (t : X) : X := smul JuliaBase.F64.log10e (TensorRing.log t)

/-- Julia `exp2(t) = exp(log(2)·t)` (AT:384). -/
@[inline] def exp2 (t : X) : X := TensorRing.exp (smul JuliaBase.F64.ln2 t)

/-- Julia `exp10(t) = exp(log(10)·t)` (AT:384). -/
@[inline] def exp10 (t : X) : X := TensorRing.exp (smul JuliaBase.F64.ln10 t)
...
@[inline] def sinc (t : X) : X :=
  if isZero t then TensorRing.one else let x := smul JuliaBase.F64.pi t; div (sin x) x

/-- Julia `cosc(t) = iszero(t) ? 0 : cos(πt)/t - sin(πt)/(πt ⟑ t)` (AT:430). -/
@[inline] def cosc (t : X) : X :=
  if isZero t then TensorRing.zero
  else let x := smul JuliaBase.F64.pi t; div (cos x) t - div (sin x) (x * t)
