...
## 2026-09-25: misc packages (DeMorgan, Fatou, AbstractAnalysis, Wilkinson)

Lean medians from `lake exe bench demorgan dendriform wilkinson fatou` after this work, Julia
from `docs/perf/latest.md` (same machine); the AbstractAnalysis rows were timed with temporary
cases (there is no `abstractanalysis` suite yet) against Julia runs of the same calls.

| case | before | after | Julia | note |
|---|---|---|---|---|
| `demorgan/tv_formula_N6` | 669 ns | 0.71 ns | 0.43 ns | `TruthValues N` is one `UInt64` word (unboxed structure), rows `2^min(N,6)` as Julia's storage; the laws and `isTautology_iff` are stated per row |
| `demorgan/truthtable_N4` | 39.2 µs | 2.79 µs | 16.5 µs | `select` compares word literals (`@[csimp]`) |
| `fatou/filled_julia_seq` | 95.8 ms | 90.1 ms | 77.2–79.5 ms | outputs preallocated and written in place (`set!`), not pushed |
