...
sl["leaf(aa,0.3)"] = encfield(Cartan.leaf(aa, 0.3))
sl["leaf(aa,1.7,1)"] = encfield(Cartan.leaf(aa, 1.7, 1))
ob = orbit(x -> x / 2 + 1, TensorField(0:0.5:1.5, x -> x * x / 7 - x / 3), [0.0, 0.5, 1.0, 1.5])
sl["orbit"] = encfield(ob)
ex = extract(aa, 2)
...
misc["besseljzero(0,1)"] = hx(besseljzero(0, 1)); misc["besseljzero(2,3)"] = hx(besseljzero(2, 3))
misc["spacing(p)"] = hx(Cartan.spacing(TensorField(0:0.25:2, poly)))
misc["spacing(a)"] = hx(Cartan.spacing(a)); misc["spacing(v)"] = hx(Cartan.spacing(v))
misc["spacing(a,1)"] = hx(Cartan.spacing(a, 1)); misc["spacing(a,2)"] = hx(Cartan.spacing(a, 2))
misc["interval_scale(t)"] = hx(Cartan.interval_scale(TensorField(0:0.25:2)))
misc["interval_scale(g2)"] = flat(Cartan.interval_scale(g2))
misc["affinepoint"] = flat(Cartan.affinepoint(points(g2)[6]))
misc["affinepoint show"] = sprint(show, Cartan.affinepoint(points(g2)[6]))
misc["concat"] = flat(Chain(1.0, 2.0) ⧺ Chain(3.0))
gb = Cartan.graphbundle(tf)
misc["graphbundle"] = encfield(gb)
misc["discontinuous"] = encfield(discontinuous(tf))
save("misc", Dict("meta" => meta, "cases" => misc))
