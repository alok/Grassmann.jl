...
### Multivector field InfoView demo

The canonical workspace exposes two small libraries above the packed runtime:

```lean
import GrassmannFields -- sampling and semantic grade records; no UI dependency
import GrassmannViz    -- validated scene and offline ProofWidgets view
```

`GrassmannFields` adapts packed `MV R3 .full` values to scalar, vector,
bivector-plane, and pseudoscalar data without exposing packed indices to a
renderer. `GrassmannViz` validates a versioned scene and embeds a local
React/SVG InfoView component. Lean computes every sample and rotor frame; the
component infers rectangular guides from the validated lattice, normalizes
display scales, constructs glyph geometry, then projects, depth-sorts, and
paints the serialized values.

Run the complete meetup preflight from the repository root:

```bash
Grassmann4/scripts/multivector_meetup_preflight.sh
```

The live anchor is `Grassmann4/MultivectorFieldDemo.lean`. See
`Grassmann4/docs/SFLeanMultivectorFieldDemo.md` for the 15-minute choreography
and recovery ladder. The checked-in
`Grassmann4/docs/MultivectorFieldFallback.svg` is generated by Lean from the
same default scene and works without an editor webview.
