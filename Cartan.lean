...
`src/quotient.jl`, `src/fiber.jl`, and the evaluation part of `src/grid.jl`): sections of trivial
bundles whose base is a structured grid (with boundary gluings from MeshTopology) or a simplex
mesh, and whose fibers are Grassmann algebra elements. Semantics, citations and the Julia defect
list: `docs/port-notes/cartan-core.md`; the Julia oracle is `oracle/cartan/gen.jl`, its defects
`oracle/cartan/defects.toml`, the tests `Tests/Cartan`.
...
#eval v.eval2 0.3 0.6                        -- Julia v(0.3, 0.6)
...
| `Cartan.ProductSpace` | `ProductSpace N` (tensor-product grids), `AffinePoint N` |
| `Cartan.Local` | `Coordinate`, `LocalTensor` = `Section` (`b ↦ f`), `LocalPrincipal` |
...
| `Cartan.Interp` | `t(x)`: multilinear evaluation through the gluings, `resample` |
| `Cartan.Product` | `FiberProductBundle` (`sb ⊕ time`), `timeParameter`, `Immersion` and Julia's accessors |
| `Cartan.Show` | display of fields and bases |
| `Cartan.Misc` | `besseljzero`, `spacing`, `interval_scale`, `affinepoint`, `⧺`, `graphbundle`, `discontinuous` |
| `Cartan.Props` | kernel-checked facts: lawful flat encodings, `get` of built fields, functor laws |
...
* **The base is a type index.** `TensorField m F` stores only its fibers; binary operations
  ...
  type checking, and fibers always have the base's length (a proof field, erased). Dynamic code
...
## Julia defects fixed (port notes §8.6, `oracle/cartan/defects.toml`)

B1 (multi-dimensional `XParameter`s throw), B2/B3 (quotient `resample`), B4 (1-D `resample`),
B7 (`ProductSpace` show of a `LinRange`), B8/B9 (`Global` eltype and size: here `MetricStore`),
B11 (`LocalTensor × LocalTensor` throws), B12 (5-D `boundarycomponents`), B18 (2-D
`HopfParameter`), B19 (`BallParameter()`/`SphereParameter()` are tubes), B25 (`angle` of a complex
field); the field product `q * v` of a quaternion and a vector field (Grassmann's
`wedgedot_metric` has the wrong sign); interpolation through the tube and cone gluings (Julia
reads the partner face positionally).
...
simplex bundles over a `DiscontinuousTopology` (a disconnected copy is `TensorField.disconnect`);
`MultilinearBundle`, `VolumeBundle`, `HomotopyBundle`; `PrincipalFiber`; `orbit`/`Limit`/
`SequenceArray` integration; the animation drivers of `variation`. The finite differences,
quadrature and differential geometry of `grid.jl`/`diffgeo.jl` are the next stage.
-/
