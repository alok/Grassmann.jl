...
@[inline] def expKernel (x : Float) : Float :=
  x * ma x (ma x (ma x (f64% 0.04166666857598777) (f64% 0.1666666857598779)) (f64% 0.4999999999999997)) (f64% 0.9999999999999912)
...
def logProc2 (f : Float) : Float :=
  ...
  Float.fma (f64% 1.0) u (Float.fma (f64% 0.0) u ((f64% 1.0) * Float.fma (Float.fma (-u) f ((f64% 2.0) * (f - u))) g q))
    ...
    let (hi, lo, x) :=
      if absx < (f64% 1.1875) then
        if absx < (f64% 0.6875) then
          ((f64% 4.63647609000806093515e-01), (f64% 2.26987774529616870924e-17), ((f64% 2.0) * absx - (f64% 1.0)) / ((f64% 2.0) + absx))
        else
          ((f64% 7.85398163397448278999e-01), (f64% 3.06161699786838301793e-17), (absx - (f64% 1.0)) / (absx + (f64% 1.0)))
      else if absx < (f64% 2.4375) then
        ((f64% 9.82793723247329054082e-01), (f64% 1.39033110312309984516e-17), (absx - (f64% 1.5)) / ((f64% 1.0) + (f64% 1.5) * absx))
      else
