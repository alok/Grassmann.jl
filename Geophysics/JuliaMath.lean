...
@[inline] def expKernel (x : Float) : Float :=
  x * ma x (ma x (ma x 0.04166666857598777 0.1666666857598779) 0.4999999999999997) 0.9999999999999912

/-- `MAGIC_ROUND_CONST(Float64)`. -/
def magic : Float := 6.755399441055744e15

/-- Julia `table_unpack(N)`: `2^(j/256)` as a high/low pair. -/
@[inline] def tableUnpack (n : Int) : Float × Float :=
  let j : UInt64 := FieldConstants.Julia.jTable[(n.emod 256).toNat]!
  (Float.ofBits ((0x3FF0000000000000 : UInt64) ||| (j &&& 0x000FFFFFFFFFFFFF)),
   Float.ofBits ((0x3C00000000000000 : UInt64) ||| (j >>> 8)))

/-- Argument reduction of `exp_impl`: `(N::Int32, r)`. -/
@[inline] def expReduce (x : Float) : Int × Float :=
  let nf := ma x 369.3299304675746 magic
  let low := nf.toBits.toUInt32.toNat
  let n : Int := if low ≥ 2 ^ 31 then (low : Int) - 2 ^ 32 else low
  let nf := nf - magic
  let r := ma nf (-0.002707606173999011) x
  let r := ma nf (-6.327543041662719e-14) r
  (n, r)

/-- `2^k · small` by adding `k` to the exponent bits (Julia's reinterpret). -/
@[inline] def scale2k (k : Int) (small : Float) : Float :=
  Float.ofBits ((FieldConstants.Julia.u64OfInt k <<< 52) + small.toBits)
...
def exp (x : Float) : Float :=
  ...
  if !(x.abs ≤ 708.3964185322641) then
    if x.isNaN then x
    else if x ≥ 709.7827128933841 then inf
    else if x ≤ -745.1332191019412 then 0.0
    else if k ≤ -53 then
      Float.ofBits ((FieldConstants.Julia.u64OfInt (k + 53) <<< 52) + small.toBits) *
...
def logProc1 (y mf bigF f : Float) : Float :=
  ...
  let u := (2.0 * f) / (y + bigF)
  let v := u * u
  let q := u * v * ma v 0.012500053168098584 0.08333333333303913
...
def logProc2 (f : Float) : Float :=
  ...
  let u := 2.0 * f * g
  let v := u * u
  let q := u * v * ma v (ma v (ma v 0.0004348877777076146 0.0022321399879194482)
...
def log1p (x : Float) : Float :=
  if x > -1.0 then
    ...
    else if -0.06058693718652422 < x && x < 0.06449445891785943 then logProc2 x
    else
      let z := 1.0 + x
      let zu := z.toBits
...
def logExt (xu : UInt64) : Float × Float :=
  ...
  let tmp := FieldConstants.Julia.i64OfU64 tmpU
  ...
  let t1 := ma k 0.6931471805598903 logc
  let t2 := t1 + r
  let lo1 := ma k 5.497923018708371e-14 logctail
  let lo2 := t1 - t2 + r
  let ar := -0.5 * r
  let ar2 := r * ar
  ...
  let hi := t2 + ar2
  let lo4 := t2 - hi + ar2
  let p := ma r (ma r (ma r (ma r (ma r 0.25001038159188854 (-0.28572740711487526))
      0.33333333317438696) (-0.3999999997661988)) 0.5000000000000007) (-0.6666666666666679)
...
def powBodyInt (x : Float) (n : Int) : Float :=
  if n == 3 then x * x * x
    ...
    if n == -2 then rx * rx
      ...
      loop rx xnlo 1.0 0.0 (-n).toNat 70
  else loop x (-0.0) 1.0 0.0 n.toNat 70
where
...
def pow (x y : Float) : Float :=
  if x.toBits == (1.0 : Float).toBits then 1.0
    ...
    if y.isNaN then y
      ...
      if yisint && yint == 0 then 1.0
      else if yisint && -4096 ≤ yint && yint ≤ 24576 then powBodyInt x yint
      else if 2 * x.toBits == 0 then
        (if y > 0 then 0.0 else inf)
        ...
        if !x.isFinite then
          ...
          (if y > 0 || x.isNaN then copysign x s else copysign 0.0 (copysign x s))
...
@[inline] def codyWaite2c (x fn : Float) (n : Int) : Int × DD :=
  let z := ma (-fn) 1.57079632673412561417e+00 x
  let y1 := ma (-fn) 6.07710050650619224932e-11 z
  let y2 := ma (-fn) 6.07710050650619224932e-11 (z - y1)
  (n, ⟨y1, y2⟩)

/-- Julia `cody_waite_ext_pio2(x, xhp)` (`rem_pio2.jl:49-87`). -/
def codyWaiteExt (x : Float) (xhp : UInt32) : Int × DD :=
  let pio21 := 1.57079632673412561417e+00
  let pio21t := 6.07710050650619224932e-11
  let pio22 := 6.07710050630396597660e-11
  let pio22t := 2.02226624879595063154e-21
  let pio23 := 2.02226624871116645580e-21
  let pio23t := 8.47842766036889956997e-32
  let fn := roundEven (x * 0.6366197723675814)
  let r := ma (-fn) pio21 x
  let w := fn * pio21t
  let j := xhp >>> 20
  let y1 := r - w
...
def paynehanek (x : Float) : Int × DD :=
  ...
  let pio2hi := 1.5707963407039642
  let pio2lo := -1.3909067614167116e-8
...
(`rem_pio2.jl:198-266`). -/
def remPio2 (x : Float) : Int × DD :=
  let xhp := poshighword x
  if xhp ≤ 0x400f6a7a then
    if (xhp &&& 0xfffff) == 0x921fb then codyWaiteExt x xhp
    else if xhp ≤ 0x4002d97c then
      if x > 0.0 then codyWaite2c x 1.0 1 else codyWaite2c x (-1.0) (-1)
    else
      if x > 0.0 then codyWaite2c x 2.0 2 else codyWaite2c x (-2.0) (-2)
  else if xhp ≤ 0x401c463b then
    if xhp ≤ 0x4015fdbc then
      if xhp == 0x4012d97c then codyWaiteExt x xhp
      else if x > 0.0 then codyWaite2c x 3.0 3 else codyWaite2c x (-3.0) (-3)
    else
      if xhp == 0x401921fb then codyWaiteExt x xhp
      else if x > 0.0 then codyWaite2c x 4.0 4 else codyWaite2c x (-4.0) (-4)
  else if xhp < 0x413921fb then codyWaiteExt x xhp
  else paynehanek x

/-- `DS1…DS6`: the sine polynomial. -/
def ds1 : Float := -1.66666666666666324348e-01
/-- sine coefficient -/ def ds2 : Float := 8.33333333332248946124e-03
/-- sine coefficient -/ def ds3 : Float := -1.98412698298579493134e-04
/-- sine coefficient -/ def ds4 : Float := 2.75573137070700676789e-06
/-- sine coefficient -/ def ds5 : Float := -2.50507602534068634195e-08
/-- sine coefficient -/ def ds6 : Float := 1.58969099521155010221e-10
/-- cosine coefficient -/ def dc1 : Float := 4.16666666666666019037e-02
/-- cosine coefficient -/ def dc2 : Float := -1.38888888888741095749e-03
/-- cosine coefficient -/ def dc3 : Float := 2.48015872894767294178e-05
/-- cosine coefficient -/ def dc4 : Float := -2.75573143513906633035e-07
/-- cosine coefficient -/ def dc5 : Float := 2.08757232129817482790e-09
/-- cosine coefficient -/ def dc6 : Float := -1.13596475577881948265e-11
...
@[inline] def cosKernelDD (y : DD) : Float :=
  ...
  let halfY2 := 0.5 * y2
  let w := 1.0 - halfY2
  w + (((1.0 - w) - halfY2) + (y2 * r - y.hi * y.lo))

/-- Julia `sin(x::Float64)` (`trig.jl:29-52`); `NaN` for infinite `x`. -/
def sin (x : Float) : Float :=
  let absx := x.abs
  if absx < 0.7853981633974483 then
    if absx < 1.4901161193847656e-8 then x else sinKernel x
...
def cos (x : Float) : Float :=
  let absx := x.abs
  if absx < 0.7853981633974483 then
    if absx < 1.0536712127723509e-8 then 1.0 else cosKernel x
  else if x.isNaN then x
  else if x.isInf then nan
  else
    let (n, y) := remPio2 x
...
def tanKernel (y : DD) (k : Float) : Float :=
  let big := y.hi.abs ≥ 0.6744
  let (yhi, ylo) :=
    if big then
      let (yhi, ylo) := if y.hi < 0.0 then (-y.hi, -y.lo) else (y.hi, y.lo)
      ((0.7853981633974483 - yhi) + (3.06161699786838301793e-17 - ylo), 0.0)
    else (y.hi, y.lo)
  let y2 := yhi * yhi
  let y4 := y2 * y2
  let r := ma y4 (ma y4 (ma y4 (ma y4 (ma y4 (-1.85586374855275456654e-05)
      7.81794442939557092300e-05) 5.88041240820264096874e-04) 3.59207910759131235356e-03)
      2.18694882948595424599e-02) 1.33333333333201242699e-01
  let v := y2 * ma y4 (ma y4 (ma y4 (ma y4 (ma y4 2.59073051863633712884e-05
      7.14072491382608190305e-05) 2.46463134818469906812e-04) 1.45620945432529025516e-03)
      8.86323982359930005737e-03) 5.39682539762260521377e-02
  let y3 := y2 * yhi
  let r := ylo + y2 * (y3 * (r + v) + ylo)
  let r := r + 3.33333333333334091986e-01 * y3
  let px := yhi + r
  if big then
    (if signbit y.hi then -1.0 else 1.0) * (k - 2.0 * (yhi - (px * px / (k + px) - r)))
  else if k == 1.0 then px
  else
    let px0 := truncLow px
    let v := r - (px0 - yhi)
    let a := -1.0 / px
...
def tan (x : Float) : Float :=
  let absx := x.abs
  if absx < 0.7853981633974483 then
    if absx < 7.450580596923828e-9 then x else tanKernel ⟨x, 0.0⟩ 1.0
  else if x.isNaN then x
  else if x.isInf then nan
  else
    let (n, y) := remPio2 x
    if n % 2 == 0 then tanKernel y 1.0 else tanKernel y (-1.0)
...
@[inline] def atanPQ (x : Float) : Float × Float :=
  let x2 := x * x
  let x4 := x2 * x2
  let p := x2 * ma x4 (ma x4 (ma x4 (ma x4 (ma x4 1.62858201153657823623e-02
      4.97687799461593236017e-02) 6.66107313738753120669e-02) 9.09088713343650656196e-02)
      1.42857142725034663711e-01) 3.33333333333329318027e-01
  let q := x4 * ma x4 (ma x4 (ma x4 (ma x4 (-3.65315727442169155270e-02)
      (-5.83357013379057348645e-02)) (-7.69187620504482999495e-02))
      (-1.11111104054623557880e-01)) (-1.99999999998764832476e-01)
  (p, q)

/-- Julia `atan(x::Float64)` (`trig.jl:504-558`). -/
def atan (x : Float) : Float :=
  let absx := x.abs
  if absx ≥ 7.378697629483821e19 then copysign 1.5707963267948966 x
  else if absx < 0.4375 then
    if absx < 7.450580596923828e-9 then x
    else
      let (p, q) := atanPQ x
      x - x * (p + q)
  else
    let xsign := if x > 0.0 then 1.0 else if x < 0.0 then -1.0 else x
    let (hi, lo, x) :=
      if absx < 1.1875 then
        if absx < 0.6875 then
          (4.63647609000806093515e-01, 2.26987774529616870924e-17, (2.0 * absx - 1.0) / (2.0 + absx))
        else
          (7.85398163397448278999e-01, 3.06161699786838301793e-17, (absx - 1.0) / (absx + 1.0))
      else if absx < 2.4375 then
        (9.82793723247329054082e-01, 1.39033110312309984516e-17, (absx - 1.5) / (1.0 + 1.5 * absx))
      else
        (1.57079632679489655800e+00, 6.12323399573676603587e-17, -1.0 / absx)
    let (p, q) := atanPQ x
    let z := hi - ((x * (p + q) - lo) - x)
    copysign z xsign
...
@[inline] def arcTRt (t : Float) : Float :=
  let p := t * ma t (ma t (ma t (ma t (ma t 3.47933107596021167570e-05
      7.91534994289814532176e-04) (-4.00555345006794114027e-02)) 2.01212532134862925881e-01)
      (-3.25565818622400915405e-01)) 1.66666666666666657415e-01
  let q := ma t (ma t (ma t (ma t 7.70381505559019352791e-02 (-6.88283971605453293030e-01))
      2.02094576023350569471e+00) (-2.40339491173441421878e+00)) 1.0
  p / q
...
def asin (x : Float) : Float :=
  let absx := x.abs
  if absx ≥ 1.0 then
    if absx == 1.0 then flipsign 1.5707963267948966 x else nan
  else if absx < 0.5 then
    if absx < 1.4901161193847656e-8 then x else ma x (arcTRt (x * x)) x
  else
    let t := (1.0 - absx) / 2.0
    let pio2lo := 6.12323399573676603587e-17
    let s := t.sqrt
    let tRt := arcTRt t
    if absx ≥ 0.975 then
      flipsign (1.5707963267948966 - (2.0 * (s + s * tRt) - pio2lo)) x
    else
      let s0 := truncLow s
      let c := (t - s0 * s0) / (s + s0)
      let p := 2.0 * s * tRt - (pio2lo - 2.0 * c)
      let q := 0.7853981633974483 - 2.0 * s0
      flipsign (0.7853981633974483 - (p - q)) x
...
def atanh (x : Float) : Float :=
  if x.isNaN then x
    ...
    let absx := x.abs
    if absx > 1.0 then nan
    else
      let t := if absx < 0.5 then log1p (2.0 * absx / (1.0 - absx))
        else log ((1.0 + absx) / (1.0 - absx))
      0.5 * copysign t x
