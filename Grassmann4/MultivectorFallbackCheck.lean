...
#eval do
  ...
  let localPath : System.FilePath := "docs/MultivectorFieldFallback.svg"
  let outerPath : System.FilePath := "Grassmann4/docs/MultivectorFieldFallback.svg"
  let fallbackPath ←
    if ← System.FilePath.pathExists localPath then pure localPath else pure outerPath
  let actual ← IO.FS.readFile fallbackPath
  ...
  let smallScene ← IO.ofExcept (defaultScene smallGrid 1 (initialSample := 0))
  let escapedScene := {
    smallScene with
    title := "A < B & \"C\""
    formula := "x > 0 & y's < 1"
  ...
  unless smallSvg.contains "A &lt; B &amp; &quot;C&quot;" &&
      smallSvg.contains "x &gt; 0 &amp; y&apos;s &lt; 1" do
